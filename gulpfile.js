...
task("styles", () => {
  //for SASS
  return (
    src(styles) //вставляем переменную со стилями
      // .pipe(gulpif(env === 'dev', sourcemaps.init())) //инициализируем SourceMap
      .pipe(concat("main.scss")) //склеиваем в один файл
...
task("scripts", () => {
  return (
    src(libs) //включаем все библиотеки в обработку
      // .pipe((env === 'dev', sourcemaps.init())) //инициализируем SourceMap
      // .pipe(concat('main.js', { newLine: ";" })) //склеиваем в один файл
